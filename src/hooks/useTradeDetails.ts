...
export function useTradeDetails(tradeId: number | null): UseTradeDetailsResult {
  ...
  useEffect(() => {
    const fetchTradeDetails = async () => {
      ...
      try {
        // Fetch trade details
        const tradeResponse = await getTradeById(tradeId.toString());
        const tradeData = tradeResponse.data;
        setTrade(tradeData);

        // Fetch related offer
        if (tradeData.leg1_offer_id) {
          const offerResponse = await getOfferById(tradeData.leg1_offer_id.toString());
          setOffer(offerResponse.data);

          // Fetch creator account
          const creatorResponse = await getAccountById(
            offerResponse.data.creator_account_id.toString()
          );
          ...
          if (tradeData.leg1_seller_account_id) {
            const sellerResponse = await getAccountById(
