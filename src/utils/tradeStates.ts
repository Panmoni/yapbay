...
export enum TradeLegState {
  ...
  /** State after the buyer has confirmed fiat payment (on-chain state). */
  FIAT_PAID = 'FIAT_PAID', // Added

  /** State after the seller has released the crypto (on-chain state). */
  RELEASED = 'RELEASED', // Added

  /** State after the trade has been completed successfully (often synonymous with RELEASED). */
  COMPLETED = 'COMPLETED',
  ...
  /** State after a dispute has been resolved by an arbitrator (on-chain state). */
 ...
 */
export const tradeStateMessages: Record<
  TradeLegState | string,
